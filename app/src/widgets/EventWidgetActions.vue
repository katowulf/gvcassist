<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <v-card-actions>
    ...
    <v-chip-group column multiple :value="selectedReactions" active-class="success">
      <v-chip v-for="emoji in reactions" :key="emoji"
              @click="toggleReaction($event, emoji)"
              :dark="card.ui.dark" outlined>
        {{ emoji }} {{reactionCounts.get(emoji) > 1? reactionCounts.get(emoji) : ""}}
      </v-chip>
    </v-chip-group>
  </v-card-actions>
</template>
  ...
  export default Vue.extend({
    ...
    methods: {
      ...
      update() {
        ...
        let i = 0;
        uids.forEach((uids: Set<string>, emoji) => {
          this.reactions.push(emoji);
          if( uids.has(myId) ) { this.selectedReactions.push(i); }
