      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-text-field label="Room description*" v-model="createForm.description" required></v-text-field>
              </v-col>
              <v-col cols="12">
                <h3>Who can access the room?</h3>
                <v-radio-group v-model="createForm.access" row>
                  <v-radio color="error" label="Anyone with link" value="link"></v-radio>
                  <v-radio color="primary" label="Email domain" value="domain"></v-radio>
                  <v-radio color="success" label="Invite only" value="whitelist"></v-radio>
                </v-radio-group>
                <v-text-field v-if="createForm.access === 'domain'"
                              label="Your domain (required)"
                              prepend-inner-icon="mdi-at" v-model="createForm.domain"
                              v-model.trim="createForm.domain"
                              :rules="createForm.rules.domain"></v-text-field>
                <v-textarea v-if="createForm.access === 'whitelist'" label="Emails to invite"
                            v-model.trim="createForm.whitelistUnformatted" rows="3"
                            @change="parseEmails" @paste="pastedEmails"></v-textarea>
                <small v-if="createForm.access === 'whitelist'">
                  {{ this.createForm.whitelist.length? this.createForm.whitelist.length + " invitees" : "You can add these later"}}
                </small>
              </v-col>
              <v-col cols="12">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="showForm.visible = false">Cancel</v-btn>
  ...
  export default Vue.extend({
    ...
    data: () => ({
      ...
      createForm: {
        ...
        rules: {
          domain: [
            v => !v || /(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]/.test(v) || "Please enter a valid domain name"
          ],
          emails: [
            v => true
          ],
